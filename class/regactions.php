 ...
 * @file    : regactions.php $
 * 
 * @id      : regactions.php | Mon Sep 14 21:14:57 2015 +0200 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
define ( 'WP_AJAX_ACTION_PREFIX', 'wp_ajax_' );
define ( 'WP_MYBACKUP_ACTION_PREFIX', 'wpmybackup_' );
define ( 'AJAX_DISPATCH_FUNCTION', WP_MYBACKUP_ACTION_PREFIX . 'ajax' );
setLanguage ( getSelectedLangCode () );
is_session_started (); 
check_is_logged (); 
$settings = loadSettings ();
if (! defined ( 'DO_NOT_AFTER_SETTINGS' ) || ! DO_NOT_AFTER_SETTINGS)
afterSettingsLoad ( $settings, true ); 
require_once CONFIG_PATH . 'default-target-tabs.php';
if (isset ( $_POST ['tlid'] ) && defined ( 'TARGETLIST_DB_PATH' )) {
...
elseif ((isset ( $_SESSION ['id'] ) && $_SESSION ['id'] == $_POST ['tlid']) || isset ( $_SESSION ['edit_step'] ) && isset ( $_SESSION ['edit_step'] ['step_data'] )) {
...
die ( sprintf ( $err_pattern, $action, _esc ( 'is badly defined (where is ' ) . get_class ( $callback [0] ) . '::' . AJAX_DISPATCH_FUNCTION . ' ?)' ) );
}
}
...
if (! $action_found)
die ( sprintf ( $err_pattern, $_POST ['action'], _esc ( 'is not defined' ) ) );
}
?>
