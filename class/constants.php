 ...
 * @file    : constants.php $
 * 
 * @id      : constants.php | Tue Nov 29 16:33:58 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
GZ => array (10,'\x1f\x8b[\x00-\x08][\x00-\x02\x04\x08\x10\x20\x40\x80][\x00-\xff]{4}[\x00\x02\x04][\x00-\x0d\xff]' ) );
...
$VERBOSITY_MODES = array (VERBOSE_MINIMAL => _ ( 'Minimal' ),VERBOSE_COMPACT => _ ( 'Compact' ),VERBOSE_FULL => _ ( 'Full' ) );
...
return implode ( DIRECTORY_SEPARATOR, array ($logs_path ['basedir'],WPMYBACKUP_LOGS,'tmp','logs','' ) );
};
...
define ( __NAMESPACE__.'\\JS_INJECT_COMMENT', PHP_EOL . '/* inject our runtime generated local (%s) functions into the global application namespace */' . PHP_EOL );
! defined ( __NAMESPACE__.'\\SORT_NATURAL' ) && define ( __NAMESPACE__.'\\SORT_NATURAL', SORT_STRING );
$open_basedir = ini_get ( 'open_basedir' );
if (! empty ( $open_basedir )) {
defined ( __NAMESPACE__.'\\OPEN_BASEDIR' ) || define ( __NAMESPACE__.'\\OPEN_BASEDIR', implode ( PATH_SEPARATOR, array_map ( function ($item) {
$item = trim ( $item );
...
('\\' == DIRECTORY_SEPARATOR) && $dir = str_replace ( array (DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR,'/' ), DIRECTORY_SEPARATOR, $dir );
...
@is_readable ( $tmp_dir ) || trigger_error ( sprintf ( _ ( 'The system temporary directory does not exist (%s)' ), $tmp_dir ), E_USER_WARNING );
?>