...
"  %{$klen}s => %s", 
$key, 
is_array( $value ) ? print_r( $value, true ) : $value ) . PHP_EOL;
} elseif ( isset( $this->method['tab'] ) )
$log_data = 'tab load [' . $this->method['tab'] . ']' .
( isset( $this->method['gr'] ) ? '[' . $this->method['gr'] . ']' : '' );
else
...
( $abort_signal = false !== ( $abort_signal_received = chkProcessSignal( $log, $sender ) ) ) &&
ackProcessSignal( $abort_signal_received[0], $abort_signal_received[1] );
return $abort_signal;
} );
die( 1 );
}
...
function auto_save() {
if ( strToBool( $this->settings['locked_settings'] ) &&
( ! isset( $this->method['locked_settings'] ) || strToBool( $this->method['locked_settings'] ) ) ) {
return;
}
$hijacked_action = isset( $this->method['supersede_action'] ) &&
in_array( $this->method['action'], explode( ',', $this->method['supersede_action'] ) );
if ( $hijacked_action ) {
$this->logfile->writeLog( 
sprintf( 
"%s - %s\n", 
date( DATETIME_FORMAT ), 
_esc( '[!] action doesn`t triggered ; it was hijacked' ) ) );
return;
}
$now = time();
...
printf( 
'<p style="color:blue"><b>' . WPMYBACKUP . '</b> (' . $version . ') %s ' .
( $is_wp ? '(Wordpress ' . $wp_version . ')' : '' ) . ' %s:</p>', 
_esc( 'is running on' ), 
_esc( 'the following system' ) );
echo '<table class="check-setup-system" style="border-spacing:0px;width:100%">';
echo '<tr><td style="font-weight:bold">' . _esc( 'OS' ) . '</td><td>:</td><td>' .
sprintf( '%s %s (%s)', php_uname( 's' ), php_uname( 'r' ), php_uname( 'm' ) ) . '</td></tr>';
echo '<tr><td style="font-weight:bold">PHP</td><td>:</td><td>' . PHP_VERSION . '</td></tr>';
echo '<tr><td style="font-weight:bold">' . _esc( 'Web server' ) . '</td><td>:</td><td>' .
$_SERVER['SERVER_SOFTWARE'] . '</td></tr>';
if ( ! empty( $cpu_info ) )
echo '<tr><td style="font-weight:bold">' . count( $cpu_info ) . 'x ' . _esc( 'CPU' ) .
'</td><td>:</td><td style="white-space: nowrap;">' . $cpu_info[0]['model name'] . '</td></tr>';
if ( ! empty( $mem_info ) ) {
$mem_info_keys = array( 
...
isset( $mem_info[$k] ) &&
printf( '<tr><td style="font-weight:bold">%s</td><td>:</td><td>%s</td></tr>', $v, $k );
}
echo '<tr><td style="font-weight:bold">' . _esc( 'URL' ) . '</td><td>:</td><td>' . stripUrlParams( 
$_SERVER['HTTP_REFERER'], 
array( 'tab' ) ) . '</td></tr>';
echo '</table>';
echo '<p style="color:blue"><b>' . WPMYBACKUP .
sprintf( 
'</b> %s</p>', 
_esc( 
...
echo '</table>';
if ( $is_wp ) {
echo "<p style='font-weight:bold'>" . sprintf( 
_esc( "Your WordPress (v%s) installation includes the following %d plugin(s) :" ), 
$wp_version, 
count( $wp_plugins ) ) . "</p>";
echo '<table class="check-setup-wpplugins"><tr class="php_setup_tbl" style="font-weight:bold"><td>' .
...
$result = array_merge( 
$result, 
array( 'cpu_info' => $cpu_info, 'mem_info' => $mem_info, 'modules' => $array ) );
return $result;
}
}
function read_alert() {
...
$samples_path = dirname( __DIR__ ) . DIRECTORY_SEPARATOR . 'custom' . DIRECTORY_SEPARATOR . 'sample' .
DIRECTORY_SEPARATOR;
$buffer = '';
...
if ( strToBool( $this->settings['locked_settings'] ) &&
( ! isset( $_POST['locked_settings'] ) || strToBool( $_POST['locked_settings'] ) ) ) {
return;
}
global $settings;
...
function ( $e ) use(&$dir_sep ) {
return strpos( $e, ' ' ) > 0 ? ( '/' == $dir_sep ? str_replace( ' ', '\\ ', $e ) : '"' . $e . '"' ) : $e;
}, 
explode( $dir_sep, $this->method['location'] ) );
$output = $ftp->deleteFile( 
implode( $dir_sep, $file_parts ), 
'del_dir' == $this->method['action'], 
true );
echo $output[0];
break;
case 'disk' :
if ( @_call_user_func( 
'del_file' == $this->method['action'] ? 'unlink' : 'rmdir', 
...
$result = $this->storage->deleteFile( 
'google' == $this->method['service'] ? _basename( $this->method['location'] ) : $this->method['location'] );
if ( is_array( $result ) ) {
...
function rst_file() {
printf( 
_esc( 'Method "%s" not yet implemented for service <b>%s</b>.<br>File location: %s' ), 
$this->method['action'], 
$this->method['service'], 
$this->method['location'] );
}
...
$ftp->ftpExecRawCmds( 
array( 'RNFR', 'RNTO' ), 
array( $old_name, _dirname( $old_name, $dir_sep ) . $dir_sep . $new_name ) );
echo 1; 
break;
case 'ssh' :
...
$ftp->ftpExecRawCmds( 
'rename', 
$old_name . ' ' . _dirname( $old_name, $dir_sep ) . $dir_sep . $new_name );
echo 1; 
break;
case 'disk' :
...
case 'webdav' :
$this->initStorage( $this->method['service'] );
$result = $this->storage->renameFile( 
'google' == $this->method['service'] ? _basename( $this->method['location'] ) : $this->method['location'], 
$this->method['new_name'] );
if ( is_array( $result ) ) {
if ( isset( $result['message'] ) && isset( $result['code'] ) )
echo $result['message'] . ' (' . $result['code'] . ')';
else
...
$ftp = getFtpObject( $this->settings, 'ssh' == $this->method['service'] );
$ftp->ftpExecRawCmds( 
array( 'ssh' == $this->method['service'] ? 'mkdir' : 'MKD' ), 
array( $this->method['location'] ) );
echo 1; 
break;
case 'disk' :
...
case 'webdav' :
$this->initStorage( $this->method['service'] );
$result = $this->storage->createFolder( 
'google' == $this->method['service'] ? _basename( $path_id ) : $path_id, 
$new_name );
if ( is_array( $result ) ) {
if ( isset( $result['message'] ) && isset( $result['code'] ) )
echo $result['message'] . ' (' . $result['code'] . ')';
...
file_put_contents( $backup_filename, json_encode( array( WPMYBACKUP_OPTION_NAME => $backup_settings ) ) );
$bak_str = sprintf( 
_esc( 'I made a backup copy before reseting them (just in case). You may find it at:%s' ), 
'<blockquote><a href=\'\' class=\'help\'>ROOT</a>/' . str_replace( ROOT_PATH, '', $backup_filename ) .
'</blockquote>' );
}
...
printf( 
"%s is no longer linked with %s.<br>You can, however, authorize the %s access anytime", 
WPMYBACKUP, 
$service_name, 
$service_name );
} else {
