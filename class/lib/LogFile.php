 ...
 * @file    : LogFile.php $
 * 
 * @id      : LogFile.php | Tue Feb 7 08:55:11 2017 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
if (in_array($this->_filter, array(
GZ,
BZ2
)) && ! _function_exists($filter . 'open'))
...
public function writeSeparator($separator = '-', $length = 80)
{
$this->writelnLog(str_repeat($separator, $length));
}
public function writelnLog($str)
{
$this->writeLog($str . PHP_EOL);
}
...
try {
$wouldblock = false;
if ('gz' == $this->_rw_filter || ! file_exists($logfile) || flock($fw, LOCK_EX | LOCK_NB, $wouldblock)) {
_call_user_func($this->_rw_filter . 'write', $fw, $log_data, strlen($log_data));
flock($fw, LOCK_UN);
} else {
if ($wouldblock) {
throw new MyException(sprintf(_esc('(1) Cannot aquire exclusive lock for writting to %s'), $logfile));
} else {
throw new MyException(sprintf(_esc('(2) Cannot aquire exclusive lock for writting to %s'), $logfile));
}
}
} finally {
...
throw new MyException(sprintf(_esc('Cannot open the file %s'), $logfile));
}
}
