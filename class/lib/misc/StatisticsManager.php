 ...
 * 
 * @id      : StatisticsManager.php | Wed Nov 30 15:38:35 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$this->_logfile->writelnLog(sprintf('[%s] %s : %s', date(DATETIME_FORMAT), $this->isSQLite() ? '' : $this->_db->get_extension(), $query));
$this->_logfile->writeSeparator();
}
$result = null;
if ($this->_is_sqlite) {
...
$result_array[] = sprintf(_esc('Result is limited to max %d rows (out of %s)'), STATISTICS_DEBUG_LOG_MAXROWS, $this->isSQLite() ? '?' : $this->_db->get_affected_rows($rst));
break;
} else
...
$result_array = $result;
$this->_logfile->writelnLog(print_r($result_array, true));
$this->_logfile->writeSeparator();
}
return $result;
}
...
$sql = 'CREATE TABLE IF NOT EXISTS ' . TBL_PREFIX . $tbl_name . '(id INTEGER NOT NULL ' . ($this->_is_sqlite ? 'PRIMARY KEY' : 'AUTO_INCREMENT') . ',' . implode(',', $col_list) . ($this->_is_sqlite ? '' : ',PRIMARY KEY(id)') . ');';
$this->_sqlExec($sql, true);
...
$result = $this->_insertRecord($tbl_name, array(
'dummyField' => 100
));
...
$record_exists_sql = 'SELECT id from ' . TBL_PREFIX . $tbl_name . (strlen($unique_cond) > 0 ? ' WHERE ' . $unique_cond : '') . ' order by id desc LIMIT 1';
...
$sql .= $key . '="' . $value . '",';
$duration = ! empty($params) && isset($params['duration']) ? $params['duration'] : (($this->_is_sqlite ? 'strftime(\'%s\',\'now\')' : 'unix_timestamp()') . '-started_time');
...
$this->_logfile->writelnLog(print_r($rst, 1) . ' is not resource');
return false;
}
if ($this->_is_sqlite)
...
return sprintf('substring_index(%s%s%s, "%s", -1) ', $quote ? '"' : '', $str, $quote ? '"' : '', '\\' . DIRECTORY_SEPARATOR);
}
}
?>