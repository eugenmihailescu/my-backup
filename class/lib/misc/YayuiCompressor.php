 ...
 * 
 * @id      : YayuiCompressor.php | Fri Mar 18 10:06:27 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
private $JS_HINTS = array( 'use strict' );
...
$this->ratio = 0;
$this->OBFUSC_SEP = array( uniqid( '_BEGIN_', true ), uniqid( '_END_', true ) );
}
...
$obfuscate_options = array( FUNCTION_ARGUMENT => true, FUNCTION_VARIABLE => true );
switch ( $set_id ) {
case MINIFY :
...
$this->_statistic[$option] = array( 'size' => 0, 'time' => 0 );
}
...
if ( $haystack[$offset] === '/' && ( ( $offset + 1 < $to && $haystack[$offset + 1] === '/' ) ||
( $offset > 0 && $haystack[$offset - 1] === '/' ) ) ) {
$in_lcomment = true;
...
if ( null === $count )
$count = 0;
$args = array( $pattern, $replacement, $subject, $limit, &$count );
$result = call_user_func_array( 'preg_replace' . ( _is_callable( $replacement ) ? '_callback' : '' ), $args );
if ( false !== $result )
$buffer = $result;
...
$es = ' ' . ( $html_separator ? '-->' : '*/' );
return sprintf( 
PHP_EOL . $bs . ( empty( $type ) ? 'Code' : $type ) . ' minified by YAYUI ~%5.2f%% (%d bytes)' . $es . '%s%s', 
...
if ( preg_match_all( '/;/', $match[0], $matches ) > 1 || preg_match_all( '/:/', $match[0], $matches ) > 0 ||
preg_match_all( '/\b(function|try|catch)\b/', $match[0], $matches ) > 0 )
return $match[0];
else
return _call_user_func( array( $obj, '_preg_replace1' ), '/[\{\}]/', ' ', $match[0], CURLY_BRACE );
};
$old_buffer = null;
while ( $old_buffer != $buffer ) {
$old_buffer = $buffer;
$buffer = $obj->_preg_replace1( 
'/\b(if|else|while)\b\.*[^;\{]*(\{)([^\{\}]+)(\})/m', 
$callback, 
$buffer, 
CURLY_BRACE );
...
$short_line = $this->_preg_replace1( 
'/\/\*([^\/]|[\w\d\s\t]\/[\w\d\s\t])*\*\//m', 
'', 
$short_line, 
BLOCK_COMMENT );
...
$v = trim( $match[0] );
if ( ! ( 1 === preg_match( '/[^\w\d]/', $v ) || in_array( $v, $this->RESERVED ) ||
is_numeric( $v ) || 0 === strlen( $v ) ) ) {
$obsf_name = strtolower( $this->_getVarName( $index++ ) );
if ( $v != $obsf_name )
do
$obfus_block['data'] = $this->_preg_replace( 
'/([\s\t\(\[\{\+\-\*\/\|<>;&%!,])(\b' . $v .
'\b)([\s\t\)\]\}\+\-\*\/\|<>;&!%,.=]?)/', 
'$1' . $obsf_name . '$3', 
$obfus_block['data'], 
...
return array( 'found' => $found > 0, 'data' => $data, 'index' => $index );
}
private function _obfuscateCodeBlock( $buffer, $index = 0 ) {
$obf_data = array( 'found' => true, 'data' => $buffer, 'index' => $index );
...
$index = 0;
if ( $this->getParamValue( $options, FUNCTION_ARGUMENT ) &&
preg_match_all( '/\bfunction\b[^\{]+/m', $func_block, $params ) > 0 ) {
foreach ( $params[0] as $vars ) {
$p1 = strpos( $vars, '(' );
$p2 = strrpos( $vars, ')' );
$items = substr( $vars, $p1 + 1, $p2 - $p1 - 1 );
$items = $this->_preg_replace1( 
'/\/\*([^\/]|[\w\d\s\t]\/[\w\d\s\t])*\*\//m', 
'', 
$items, 
BLOCK_COMMENT );
...
$v = trim( $match[0] );
if ( ! ( 1 === preg_match( '/[^\w\d]/', $v ) || in_array( $v, $this->RESERVED ) || is_numeric( $v ) ||
0 === strlen( $v ) ) ) {
$obsf_name = strtolower( $this->_getVarName( $index++ ) );
if ( $v != $obsf_name )
do
$func_block = $this->_preg_replace( 
'/([^\w\d\'".]*)(\b' . $v . '\b)([^\w\d\'"]*)/', 
'$1' . $obsf_name . '$3', 
$func_block, 
- 1, 
$rc, 
FUNCTION_ARGUMENT ); while ( $rc > 0 );
else
$index--;
}
}
...
$minify = ( $this->getParamValue( $options, JSHINTS ) || $this->getParamValue( $options, SEMICOLON ) ||
$this->getParamValue( $options, SINGLE_COMMENT ) || $this->getParamValue( $options, BLOCK_COMMENT ) ||
$this->getParamValue( $options, CURLY_BRACE ) || $this->getParamValue( $options, WHITESPACE ) );
$obfuscate = ( $this->getParamValue( $options, FUNCTION_VARIABLE ) ||
$this->getParamValue( $options, FUNCTION_ARGUMENT ) );
if ( $minify || $obfuscate ) {
