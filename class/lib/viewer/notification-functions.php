 ...
 * 
 * @id      : notification-functions.php | Tue Feb 16 15:27:30 2016 +0100 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
if (false !== ($message = $alert_message_obj->getMessageById ( $method ['msg_id'] )))
$alert_message_obj->setReadStatus ( $message->msg_id, ! $message->status );
elseif (in_array ( $method ['msg_id'], array (
'read',
'unread' 
) ))
...
if (empty ( $arrow ) && $show_scrollbar && $i - $offset == $slider_row && $i + 1 - $offset != $get_scrollbar_row ( $i + 1, $count, $max_top )) {
$draw_slider = true;
$bg = ';background-color:#00ADEE';
$tooltip = _esc ( 'Click the scrollbar to move the slider' );
} else 
if ($show_arrow && ! empty ( $arrow ))
$onclick = "onclick='jsMyBackup.messages_scroll(" . ($offset == $i ? '-1' : ($i > $count ? '0' : '1')) . ");' onmousedown='var sender=this;jsMyBackup.globals.slider_down=setInterval(sender.onclick,200);' onmouseup='clearInterval(jsMyBackup.globals.slider_down);'";
...
$log = MESSAGES_LOGFILE;
$alert_message_obj = new MessageHandler ( $log );
$fill_bug_report = sprintf ( _esc ( "If that`s not the case and you suspect a code bug then please %s." ), "<a href='" . getReportIssueURL () . "'>" . _esc ( 'fill a report' ) . "</a>" );
if (intval ( $method ['msg_id'] ) > 0) {
if (false === ($msg_item = $alert_message_obj->getMessageById ( $method ['msg_id'] )))
die ( sprintf ( _esc ( "No detail found. That`s rather odd! Please try again by %s.<br>%s" ), "<a href='#' onclick='location.reload(true);'>" . _esc ( 'reloading the page' ) . "</a>", $fill_bug_report ) );
} else
return; 
$types = $alert_message_obj->getMessageTypes ();
$help_1 = "'" . _esc ( "Normally here you should have the job id that had generated this message.<br>My guess is that the job history was flushed so this record does not contain<br>anything about this event." ) . "<br>" . str_replace ( "'", "\\'", $fill_bug_report ) . "'";
$help_2 = "'" . sprintf ( _esc ( "A message is marked as read/unread only when you explicitly click the<br>icon (eg: %s) that precedes the message.<br>Note that the messages that are read are not deleted permanently. Instead<br>they are filtered out such that you can find them in <i>read messages</i>." ), "<img src=\\'" . plugins_url_wrapper ( 'img/msg_error.png', IMG_PATH ) . "\\'>" ) . "'";
$style = array ();
if (! empty ( $types [$msg_item->type] ['fg'] ))
$style [] = '
