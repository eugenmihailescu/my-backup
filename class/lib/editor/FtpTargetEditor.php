 ...
 * 
 * @id      : FtpTargetEditor.php | Mon Sep 14 21:14:57 2015 +0200 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
$this->java_scripts [] = 'function validateSSLCAInfo(sender){if(!sender)return;var b=sender.options[sender.selectedIndex].value,c=document.getElementById("ftp_lib"),t1=document.getElementById("ftp_cainfo_table");if(c){t1.setAttribute("style","display:"+("' . (CURLPROTO_FTP | CURLPROTO_FTPS) . '"== b && "curl"==c.options[c.selectedIndex].value?"inline-block":"none"));toggle_passive(document.getElementById("ftppasv"));}}validateSSLCAInfo(document.getElementById("ftpproto"));';
if (isWin ())
$this->java_scripts [] = 'function validateSSLonWin(sender){if(!sender)return;var b=sender.options[sender.selectedIndex].value,c=document.getElementById("ftp_lib");if(c)if("' . (CURLPROTO_FTP | CURLPROTO_FTPS) . '"==b && c.options[c.selectedIndex].value=="php"){parent.popupError("Warning","The official PHP build does not support out of the box the FTP over SSL functionality (' . readMoreHere ( 'http://php.net/manual/en/function.ftp-ssl-connect.php',null,'_blank',true ) . ').<br>However, you may opt for Curl library which supports all kind of stuff, including FTPS :-)");}}';
}
private function _getJavaScripts() {
$this->java_scripts [] = 'function toggle_passive(sender){if(!sender)return;var c=document.getElementById("ftp_active_port_tbl"),d=document.getElementById("ftp_lib");if(c&&d)c.setAttribute("style","display:"+(sender.checked||"curl"!=d.options[d.selectedIndex].value?"none":"inline-block"));}toggle_passive(document.getElementById("ftppasv"));';
...
return empty ( $this->_ftphost ) || empty ( $this->_ftpuser ) || empty ( $this->root ) || ! $this->enabled || 'sftp' == $this->_ftpproto;
}
...
$systype = 'unix';
$java_scripts = echoFolder ( $this->target_name, $this->root, $this->root, $this->ext_filter, $this->function_name, getFtpDirSeparator ( $systype, $this->_dirsep ), preg_match ( '/^win/i', $systype ) ? null : ($direct_dwl ? 'ftp://<ftpuser>:<ftppwd>@<ftphost>:<ftpport>/<ftp>' : null), $this->folder_style, false, null, $this->settings ); 
...
$help_1 = "'" . _esc ( 'Specify the directory listing type used by the remote FTP server.<br>Microsoft IIS FTP Server can be configured to list both style Windows/Unix.<br>This file list does not depend on that. Default is Unix-like style.' ) . '<br>' . readMoreHereE ( 'http://support.microsoft.com/kb/256312' ) . "'";
$help_2 = "'" . sprintf ( _esc ( '<b>What is what</b><ol><li>FTP - the %s that has been around since 1970s. It usually runs over TCP port 21</li><li>%s - an extension to the FTP that adds support for the TLS and SSL. FTPS should not be confused with the SSH File Transfer Protocol (SFTP).</li><li>%s - another, completely different file transfer protocol that has nothing to do with FTP. SFTP runs over an SSH session, usually on TCP port 22. It has been around since late 1990s.</li><li>%s - is a means of securely transferring computer files between a local host and a remote host or between two remote hosts. It is based on the SSH protocol</li></ol>' ), getAnchorE ( 'plain old FTP protocol', 'http://en.wikipedia.org/wiki/File_Transfer_Protocol' ), getAnchorE ( 'FTPS=FTP+SSL', 'http://en.wikipedia.org/wiki/FTPS' ), getAnchorE ( 'SFTP=SSH+FTP', 'http://en.wikipedia.org/wiki/File_Transfer_Protocol#SFTP' ), getAnchorE ( 'SCP (Secure Copy Protocol)', 'http://en.wikipedia.org/wiki/Secure_copy' ) ) . "'";
