 ...
 * 
 * @id      : OAuthTargetEditor.php | Wed Nov 30 15:38:35 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$this->java_scripts[] = "var div=document.getElementById('{$this->target_name}_authorize_div');if(div){div.style='width:0px;height:0px;visibility:hidden';}" .
"parent.plugin_dir='" . addslashes( dirname( realpath( $_SERVER['SCRIPT_NAME'] ) ) ) . "';";
}
...
bindSSLInfo( 
$this->target_name, 
$_SESSION[$this->target_name . '_ssl_cert_info'], 
$this->java_scripts, 
$ssl_hint );
} catch ( MyException $e ) {
}
...
if ( isset( $_GET[$this->target_name . '_unlink'] ) && _file_exists( $target_auth_file ) ) {
unlink( $target_auth_file );
$this->java_scripts[] = "jsMyBackup.popupWindow('" . _esc( 'Success' ) . "','" . sprintf( 
_esc( "%s is no longer linked with %s.<br>You can, however, authorize the %s access anytime" ), 
WPMYBACKUP, 
$this->target_name_name, 
$this->target_name_name ) . "');";
}
$this->_authInfo = null;
if ( _file_exists( $target_auth_file ) ) {
...
if ( getParam( $this->settings, $this->target_name . '_enabled' ) ) {
printf( 
'<div class="highlight-box hintbox %s" style="display:inline">%s</div>', 
$this->container_shape, 
_esc( 'This backup destination is enabled but not yet authorized.' ) );
}
require_once OAUTH_PATH . $session_class . '.php';
echo "<!-- Storage-Cloud auth-box -->" . PHP_EOL;
echo '<div id="storage_authorize_div"><p class="redcaption">' . sprintf( 
_esc( 
'%s is not yet configured</p><p>Before using this option you have to authorize %s to upload files to your %s account :' ), 
$this->target_name_name, 
WPMYBACKUP, 
$this->target_name_name );
echo '<input type="button" class="button-primary" name="btnSubmit" value="' . _esc( 'Authorize' ) .
'" onclick="jsMyBackup.send_oauthrequest(\'post\',\'' . $this->target_name . '\',\'' .
htmlspecialchars( PROXY_PARAMS ) . '\');">';
if ( 'google' == $this->target_name )
echo '<br>' . readMoreHere( 
'https://developers.google.com/accounts/docs/OAuth2WebServer#offline', 
sprintf( _esc( 'about %s authorization scope' ), $this->target_name_name ) );
echo '</div>' . PHP_EOL;
echo "<!-- Storage-Cloud auth-box -->" . PHP_EOL;
}
require_once OAUTH_PATH . $session_class . '.php';
...
if ( ! ( null == $this->_authInfo || _file_exists( $target_auth_file ) ) ) {
file_put_contents( $target_auth_file, json_encode( $this->_authInfo ) );
locationRedirect( $this->_stripOAuthFromURL() );
}
} catch ( MyException $e ) {
$this->_authInfo = null;
echo "<div class='hintbox {$this->container_shape}' style='display:inline-block'>" .
_esc( 'An unexpected error occurred while tried to authenticate at ' ) . ucwords( $this->target_name ) .
":<br><p class='redcaption'>" . $e->getMessage() . '</p>';
echo '<b>' . _esc( 'Solution:' ) . '</b>';
printf( 
_esc( 
'<p>Try again (eventually few more times at an interval of 60s or so). If it doesn`t work then %sclick here</a>.</p>' ), 
'<a href="#" onclick="jsMyBackup.asyncGetContent(jsMyBackup.ajaxurl,\'' . http_build_query( 
array( 
'action' => 'del_oauth', 
...
printf( 
_esc( '<p>If whatever you try doesn`t work then please %sreport this incident</a>.</p>' ), 
'<a href="' . getTabLink( $TARGET_NAMES[APP_SUPPORT] ) . '&support_category=error">' );
echo '</div>';
}
return null != $this->_authInfo;
}
