 ...
 * 
 * @id      : ScheduleEditor.php | Wed Nov 30 15:38:35 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$onclick = "onclick=\'jsMyBackup.popupWindow(&quot;Help $str&quot;,&quot;{$this->_options_help[$str]}&quot;);\'";
else
$onclick = '';
return '<span class=\'help schedule_param\' ' . $onclick . '>' . $str . '</span>';
}
protected function _echoParam( 
$name, 
$long_name, 
$include_empty = false, 
$default = null, 
...
echo '=</b>' . $q . $cs . ( empty( $value ) ? ( empty( $default ) ? '' : $default ) : addslashes( $value ) ) .
$ce . $q;
else
echo '</b>';
}
...
global $REGISTERED_SCHEDULE_TABS, $settings;
$settings = $this->settings;
$sel_tab = getSelectedTab();
$tab_link = getTabLink( $sel_tab );
$tab_link = stripUrlParams( $tab_link, array( 'gr' ) );
$tabs = array();
...
echo "<li class='$class' style='margin: 0 2px;border-left:1px solid;border-right:1px solid;'><a href='" .
$href . "' style='padding-top:5px;padding-bottom:5px;'>$name</a>";
}
echo '</ul></div>';
echo '<div id="content-container1" class="content-container horizontal ' . $this->container_shape .
'" style="min-height: 500px;">';
if ( ! isset( $REGISTERED_SCHEDULE_TABS[$group] ) ) {
if ( false !== ( $include_tab_file = chkIncludeTab( 
$tabs, 
$group, 
basename( CUSTOM_PATH ) . DIRECTORY_SEPARATOR . 'targets' ) ) )
