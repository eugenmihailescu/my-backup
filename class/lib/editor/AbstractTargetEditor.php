 ...
 * 
 * @id      : AbstractTargetEditor.php | Tue Feb 16 21:41:51 2016 UTC | Eugen Mihailescu eugenmihailescux@gmail.com $
*/

namespace MyBackup;

define( __NAMESPACE__.'\\TARGET_TABLE_STYLE', '' ); 
...
if ( isset( $this->settings['compression_type'] ) )
if ( isset( $COMPRESSION_NAMES[$this->settings['compression_type']] ) )
$this->ext_filter = $COMPRESSION_NAMES[$this->settings['compression_type']];
else
$this->ext_filter = $COMPRESSION_NAMES[BZ2]; 
if ( isset( $this->settings[$this->target_name] ) ) {
$root = $this->settings[$this->target_name];
$this->root = normalize_path( $root, true );
}
isset( $this->settings[$this->target_name . '_enabled'] ) &&
$this->enabled = strToBool( $this->settings[$this->target_name . '_enabled'] );
$this->enabled_tag = $this->enabled ? '' : ' disabled';
isset( $this->settings[$this->target_name . '_age'] ) &&
$this->age = $this->settings[$this->target_name . '_age'];
$this->is_wp = is_wp();
...
echo sprintf( 
'<tr><td colspan="3"><p style="color:red">%s</p></td></tr>', 
sprintf( _esc( 'Cannot render this section due to missing template file (%s).' ), $template_file ) );
printf( 
'<tr><td colspan="3">' .
_esc( 'Please reinstall the product. If the error persists please %sfill an error report' ) .
'</a>.</td></tr>', 
...
echo '<table name="' . strtolower( get_class( $this ) ) . '"' . TARGET_TABLE_STYLE . '>' . $editor_template .
'</table>' . PHP_EOL;
if ( $close_divs )
...
isset( $fields['error_code'] ) && isset( $fields['error_message'] ) && $this->java_scripts[] = 'parent.popupError("' .
_esc( 'Error detected' ) . '","' . sprintf( 
_esc( 'It seems that the last request sent encountered a problem:%s' ), 
'<br>' . $fields['error_message'] . '(' . $fields['error_code'] . ')' ) . '");';
...
$this->java_scripts[] = "parent.asyncGetContent(parent.ajaxurl,'" . http_build_query( 
array( 
...
sprintf( 
_esc( 'Class %s has not called parent %s::initTarget method. This is a MUST by design.' ), 
get_class( $this ), 
get_class() ) );
...
echo '<br>' . PHP_EOL;
echo '<div id="folder_info" ' .
( null == $this->infoBannerCSSStyle ? '' : ( 'style="' . $this->infoBannerCSSStyle . '"' ) ) .
' class="' . ( null == $this->infoBannerCSSClass ? 'folder_info' : $this->infoBannerCSSClass ) .
'"></div>' . PHP_EOL;
}
if ( ! $this->hasCustomFrame )
echo '</div>' . PHP_EOL . '</div>' . PHP_EOL;
