 ...
 * 
 * @id      : BackupSettingsEditor.php | Tue Feb 16 15:27:30 2016 +0100 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
getAnchorE( _esc( 'easily convert the .crt to .pem format' ), lmgtfy( '.crt to .pem' ) ), 
getAnchorE( 
_esc( 'certificate directory is prepared using the openssl c_rehash utility' ), 
'http://curl.haxx.se/libcurl/c/CURLOPT_CAPATH.html' ) );
$help_1 .= '<p style=\\\'font-weight:bold\\\'>' .
sprintf( 
_esc( 
...
$help_4 = "'" .
_esc( 
'If you are using Curl library and you connect the Internet through a cache proxy server then specify the proxy server IP/name and its respective port (like 3128).' ) .
"'";
$help_5 = "'" .
_esc( 
'If you have specified a proxy server above and if the server allows only authenticated users to connect it then please enter the user/password for proxy. Your system administrator should help you with these.' ) .
"'";
$help_6 = "'" .
_esc( 
'With Curl library there are only two proxy authentication methods supported: the Basic and Windows NTLM.' ) .
"'";
$help_7 = "'" .
_esc( 
'If you choose to connect the Internet via proxy then specify the proxy type you connect. If in doubt then choose HTTP proxy. For more info ask your system administrator.' ) .
"'";
$help_8 = "'" .
sprintf( 
_esc( 
'This is a quote from %sWhen negotiating a TLS or SSL connection, the server sends a certificate indicating its identity. Curl verifies whether the certificate is authentic, i.e. that you can trust that the server is who the certificate says it is. This trust is based on a chain of digital signatures, rooted in certification authority (CA) certificates you supply </i>(see CA PEM path/file parameter). <i>When this option is checked, and the verification fails to prove that the certificate is authentic, the connection fails. When the option is unchecked, the peer certificate verification succeeds regardless.%s: disabling verification of the certificate allows bad guys to %s the communication without you knowing it.</div>' ), 
' ' . getAnchorE( 
_esc( 'libcurl manual' ), 
'http://curl.haxx.se/libcurl/c/CURLOPT_SSL_VERIFYPEER.html' ) . ':<blockquote><i>', 
'</i></blockquote><div class=\\\'hintbox ' . $this->container_shape . '\\\'><b>' . _esc( 'WARNING' ) .
'</b>', 
getAnchorE( 
_esc( 'man-in-the-middle (MitM)' ), 
...
$help_10 = "'" .
_esc( 
'If you set this option then, when using SSL, it is checked (1) the existence of a common name on the SSL certificate and also (2) verify that it matches the hostname provided. The most users should set this option ON.' ) .
"'";
$help_11 = "'" .
_esc( 
'The name of the outgoing network interface to use. This can be an interface name, an IP address or a host name. By default the system`s default interface is used.' ) .
"'";
