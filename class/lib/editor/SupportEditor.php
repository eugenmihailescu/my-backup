 ...
 * 
 * @id      : SupportEditor.php | Mon Sep 14 21:14:57 2015 +0200 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
$this->java_scripts [] = "parent.print_debug_sample=function(type){parent.asyncGetContent(parent.ajaxurl,'action=print_debug_sample&type='+type+'&nonce=" . wp_create_nonce_wrapper ( 'print_debug_sample' ) . "','__dummy__',function(xmlhttp){js55f846e1d1da3.popupWindow('Sample '+type+' log','<div class=\'cui-console\'><pre>'+xmlhttp.responseText+'</pre></div>');});}";
...
$php_min_ver = '<a class="help" onclick=' . getHelpCall ( "'" . sprintf ( _esc ( "Please don`t bother using any version older than PHP %s. Their %s occured like %d years ago, are very,very buggy and they have lots of security holes. That`s why I don`t bother writting software for anything else than PHP %s or newer :-)<br>Make sure your web hosting company provides you at least the version %s (which btw ended also like %.1f years ago)." ), SUPPORT_MIN_PHP, getAnchorE ( 'End-Of-Life', 'http://php.net/eol.php' ), time () / 31536000 - 41.03, SUPPORT_MIN_PHP, SUPPORT_MIN_PHP, time () / 31536000 - 44.7 ) . "'" ) . '>PHP ' . SUPPORT_MIN_PHP . '+</a>';
$wp_min_ver = is_wp () ? (' ' . _esc ( 'and' ) . ' ' . '<a class="help" onclick=' . getHelpCall ( "'" . sprintf ( _esc ( 'This application was tested and works with all version of WordPress 3.0 to 4.1.<br>%s the majority of WordPress installations (ie. 99.9%%) runs on WordPress version 3.0 or newer. The same source states that 75%% of these installations run on PHP 5.3 or newer.<br>We expect that this application will install and run without problems on any of these versions.' ), getAnchorE ( 'According to wordpress.org', 'https://wordpress.org/about/stats' ) ) . "'" ) . '>WordPress ' . SUPPORT_MIN_WP . '+</a>') : '';
...
if (! $this->_is_activated)
return;
$help_debug_on = "'" . _esc ( 'This option activates a mechanism by which when an error occurs (no matter if it`s an program error or just an exception, like I/O error, HTTP timeout, etc) its trace is automatically captured and saved into a special trace log file. <a class=\\\'help\\\' href=\\\'#\\\' onclick=\\\'js55f846e1d1da3.print_debug_sample(&quot;debug&quot;);\\\'>' . _esc ( 'See example' ) . '</a>.' );
...
$help_stats_debug_on = "'" . _esc ( 'This option will dump to a log file the JavaScript code that represents the datase sent to Google Charts API in order to get back a user-friendly chart.' ) . '<br><a class=\\\'help\\\' href=\\\'#\\\' onclick=\\\'js55f846e1d1da3.print_debug_sample(&quot;stats&quot;);\\\'>' . _esc ( 'See example' ) . ".</a>'";
