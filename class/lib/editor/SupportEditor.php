 ...
 * 
 * @id      : SupportEditor.php | Tue Feb 16 15:27:30 2016 +0100 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
SUPPORT_MIN_PHP, 
SUPPORT_MIN_PHP, 
time() / 31536000 - 44.7 ) . "'" ) . '>PHP ' . SUPPORT_MIN_PHP . '+</a>';
$wp_min_ver = is_wp() ? ( ' ' . _esc( 'and' ) . ' ' . '<a class="help" onclick=' . getHelpCall( 
"'" . sprintf( 
_esc( 
'This application was tested and works with all version of WordPress 3.0 to %s.<br>%s the majority of WordPress installations (ie. 99.9%%) runs on WordPress version 3.0 or newer. The same source states that 90%% of these installations run on PHP 5.3 or newer.<br>We expect that this application will install and run without problems on any of these versions.' ), 
get_bloginfo( 'version', 'display' ), 
getAnchorE( 'According to wordpress.org', 'https://wordpress.org/about/stats' ) ) . "'" ) . '>WordPress ' .
SUPPORT_MIN_WP . '+</a>' ) : '';
$iis_apache_php = sprintf( 
_esc( 'over IIS/Apache with MySQL v%s+, %s%s' ), 
SUPPORT_MIN_MYSQL, 
$php_min_ver, 
$wp_min_ver );
...
return;
$help_debug_on = "'" . _esc( 
'This option activates a mechanism by which when an error occurs (no matter if it`s an program error or just an exception, like I/O error, HTTP timeout, etc) its trace is automatically captured and saved into a special trace log file. <a class=\\\'help\\\' href=\\\'#\\\' onclick=\\\'jsMyBackup.print_debug_sample(&quot;debug&quot;);\\\'>' .
_esc( 'See example' ) . '</a>.' );
$help_debug_on .= _esc( 
...
$help_curl_debug_on .= sprintf( 
_esc( 
"Oh btw, if you wonder what the heck is Curl, %s is just a software library (written by %s and other - thanks Daniel+others!) that helped me to write the code that takes care of sending and receiving data over Internet, like uploading a backup to Dropbox and so on. It is %s that there is no mistery why it become so popular among the PHP community and not only." ), 
getAnchorE( 'Curl', 'http://en.wikipedia.org/wiki/CURL' ), 
getAnchorE( 'Daniel Stenberg', 'http://daniel.haxx.se' ), 
getAnchorE( 'so powerful', 'http://curl.haxx.se/docs/comparison-table.html' ) );
$CurlErrorCode = _esc( 'libCURL Error Codes' );
$help_curl_debug_on .= '<p style=\\\'font-weight:bold\\\'>' . $CurlErrorCode . '</p>' . sprintf( 
_esc( 'Whenever you find a CURL error which code you do not understand please check the %s.' ), 
getAnchorE( $CurlErrorCode, 'http://curl.haxx.se/libcurl/c/libcurl-errors.html' ) );
$help_curl_debug_on .= '<p style=\\\'font-weight:bold\\\'>' . sprintf( 
_esc( 
'Let`s make the long story short</p>Activate this option (likewise Debug ON) either when you want to report a problem OR when you want to dive deeper into %s and/or protocol debugging and understanding :-)' ), 
getAnchorE( 'libcurl', 'http://curl.haxx.se/libcurl' ) );
$help_curl_debug_on = "'$help_curl_debug_on '";
$help_yayui_on = getAnchorE( 'YAYUI', 'http://yayui.mynixworld.info' ) . sprintf( 
_esc( 
'%s is a small library (I wrote a while ago) that enhances the page loading speed by reducing the page length just before the page is dispatched from your webserver towards your web browser. It strips the unnecessary whitespaces, comments and even obfuscates the JavaScript code making it shorter thus `faster`. The only benefit of this option would be that %s administrative pages will load faster and will drain less RAM from your local system.<br>' ), 
'(Yet Another YUI compressor)', 
WPMYBACKUP );
$help_yayui_on .= _esc( 'If you`re in doubt then leave this option off. It won`t harm either way.' );
$help_yayui_on = "'$help_yayui_on '";
$help_debug_statusbar_on = "'" .
_esc( 
'This option will display a status bar that will stick at the bottom of the page. It will display how long took the webserver to produce the page, how many bytes were received by browser, how much did the YAYUI reduced the content size, how long took the browser to render the document and to load all the remote resources. It also display the total time from the moment the server received the request till the moment you saw the page on screen.' ) .
"'";
$help_stats_debug_on = "'" .
_esc( 
...
'<br><a class=\\\'help\\\' href=\\\'#\\\' onclick=\\\'jsMyBackup.print_debug_sample(&quot;stats&quot;);\\\'>' .
_esc( 'See example' ) . ".</a>'";
...
$help = 'help_' . $op . '_on';
$tr[] = '<tr><td ' . ( 0 == $i++ ? 'style="width:0px"' : '' ) . '><label for="' . $op . '_on">' . $op_name .
'</label></td><td><input type="checkbox" name="' . $op . '_on" id="' . $op . '_on" value="1" ' .
( strToBool( $this->settings[$op . '_on'] ) ? 'checked' : '' ) . '><input type="hidden" name="' . $op .
'_on" value="0"><a class="help" onclick=' . getHelpCall( $$help ) . '> [?]</a></td></tr>';
}
echo '<tr><td colspan="2">' .
_esc( 
'This is a complex application. Sometimes shits just happens and thus it is always a good idea to have a first aid kit nearby.' ) .
'</td></tr>';
