 ...
 * 
 * @id      : BackupJobEditor.php | Tue Dec 13 06:40:49 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$this->java_scripts[] = "parent.toggle_compression_type=function(sender) {" . $level_support . "can_level='undefined'!=typeof support_level[sender.options[sender.selectedIndex].value];if(!can_level)e.value = '0';e.disabled = !can_level;}";
...
$on_backup_click = sprintf("if(!jsMyBackup.enabled_backup_targets.length)return jsMyBackup.popupError('%s','%s');jsMyBackup.asyncRunBackup('%s','%s','%s','%s','%s','%s',null,null,jsMyBackup.onJobDone);", _esc('Error'), sprintf(_esc('You have not selected any %s option.'), getTabAnchorE(APP_TABBED_TARGETS)), $backup_action, _esc('Backup'), wp_create_nonce_wrapper($backup_action), wp_create_nonce_wrapper('get_progress'), wp_create_nonce_wrapper('cleanup_progress'), wp_create_nonce_wrapper('abort_job'));
...
$compress_cmd = getTarNZipCmd($this->settings['dir'], addTrailingSlash($this->settings['wrkdir']) . ('' == $this->settings['name'] ? $this->settings['url'] . "-yymmdd-hhiiss" : $this->settings['name']), $this->settings['compression_type'], $this->settings['compression_level'], MB * $this->settings['size'], $excl_files, explode(',', $this->settings['excludedirs']), explode(',', $this->settings['excludeext']), strToBool($this->settings['excludelinks']), $this->settings['bzipver'], $this->settings['cygwin']);
...
if (IS_MULTISITE && ! SANDBOX)
$help_3 .= '<br>' . sprintf(_esc('On multisite installation this location is set automatically. The network administrator may change it globally on the %s page (see `Global working directory` option).'), getAnchor(_esc('Network Settings'), network_admin_url('settings.php') . '#' . WPMYBACKUP_LOGS, null, true));
$help_3 = "'" . $help_3 . "'";
$help_4 = "'" . _esc('If the backup file size reaches this threshold then it will be splitted<br>into multiple volumes whose size is less than or equal to this number.') . '<br>' . _esc('When 0 (zero) then no volume limit thus no spanning.') . "'";
...
$help_6 .= '<br><br>' . sprintf(_esc('<b>Compression ratio</b> : %s (the lower the better)'), getSpanE('BZip2', 'green', 'bold') . ' < GZip < Zip|PclZip < LZF');
$help_6 .= '<br><b>' . sprintf(_esc('Compression speed</b> : %s (the higher the better)'), getSpanE('LZF', 'green', 'bold') . ' > GZip > Zip > BZip2 > PclZip');
...
defined(__NAMESPACE__.'\\BACKUP_MODE_INC') && $modes_desc[BACKUP_MODE_INC] = 'Only those files will be included which have been changed since the last backup.';
defined(__NAMESPACE__.'\\BACKUP_MODE_DIFF') && $modes_desc[BACKUP_MODE_DIFF] = 'Only those files will be included which have been changed since the last FULL backup.';
$backup_modes = '';
$help_4 = '';
...
$help_1 = "'" . _esc('On Windows the external compression tool works on top of CygWin.<br>You should specify here the fullpath of <b>bash.exe</b> that comes with CygWin.<br>Additionally, you should specify here any parameter that bash.exe should use by default.<br><br><b>Example</b>: c:\\\\cygwin\\\\bin\\\\bash.exe --login -c') . "'";
...
$help_3 = "'" . _esc('The number of miliseconds to sleep the CPU between each compression cycle.') . '<br>' . _esc('It throttles the CPU usage so other processes can take advantage of it.') . '<br>' . _esc('To disable this feature set this option to zero.') . '<br>1 sec = 1000 ms' . "'";
...
$help_7 = "'" . sprintf(_esc('Set the number of seconds the backup job is allowed to run. If this is reached, the script returns a fatal error. The default limit is %s seconds. If set to zero, no time limit is imposed.') . '<br>' . _esc('This option has no effect when PHP is running in safe mode.'), $factory_options['backup']['max_exec_time'][0]) . "'";
...
$help_9 = "'" . _esc('The memory limit the PHP script may use. 128MiB should be enough but if you notice that the backup script fails due to `Allowed memory size of XXX bytes exhausted` then try to increase this value.<br>Depending on what plug-ins you have installed and what they are doing the memory used by the backup script may vary. For instance, if you have a profiling plug-in (eg. P3) which monitors what the other plug-ins do then the memory used will be considerably higher (required not by our plug-in functionalities but by the profiling plug-in which traces every call of our plug-in).') . "'";
$help_10 = "'" . _esc('When this option is set then the file names within the backup archives will include only the website relative path and not the file absolute/real path.') . getExample(_esc('Example'), _esc('If a file fullpath is /some-path/website-root/include/readme.txt then the relative path of the file would be website-root/include/readme.txt'), false) . "'";
$help_11 = "'" . _esc('This option automatically creates a safe copy of any individual plug-ins/themes just before Wordpress updates them.') . "'";
