 ...
 * 
 * @id      : GenericCloudStorage.php | Mon Sep 14 21:14:57 2015 +0200 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
if (null != $header)
$hdr = (is_array ( $header ) ? $header : array (
$header 
)) + $hdr;
}
}
...
$error = sprintf ( _esc ( 'File "%s" does not exists on local disk. Cannot upload to %s.' ), $filename, $service_name );
...
$error = sprintf ( _esc ( 'The size of the file %s (%d bytes) exceeds the %s upload limit (%d bytes).' ), $filename, $file_size, $service_name, $upload_limit );
} elseif (empty ( $error ))
if ($file_size > ($free_space = $this->_getFreeSpace ()))
$error = sprintf ( _esc ( 'The size of the file %s (%d bytes) exceeds the %s free space (%d bytes).' ), $filename, $file_size, $service_name, $free_space );
if (! empty ( $error ))
...
return $this->_genericPost ( 'delete', $url, $postfields, $method, null, function ($e) {
if (204 == $e->getCode ())
return true;
return null;
} );
...
return ! (empty ( $e )) ? json_encode ( array (
