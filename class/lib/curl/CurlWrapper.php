 ...
 * 
 * @id      : CurlWrapper.php | Tue Feb 16 15:27:30 2016 +0100 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
return extension_loaded( 'curl' ) && ( ( $version = curl_version() ) && isset( $version['protocols'] ) &&
( ! isset( $url_parts['scheme'] ) || in_array( $url_parts['scheme'], $version['protocols'] ) ) );
}
...
CURLOPT_TIMEOUT => 0,  
CURLOPT_BINARYTRANSFER => true ) + $cmd_options;
$obj = &$this;
...
$cmd_options = array( CURLOPT_TIMEOUT => 0, 				
CURLOPT_BINARYTRANSFER => true ) + $cmd_options;
$obj = &$this;
if ( _is_callable( $this->onAbortCallback ) ) {
...
$factory_options = array( CURLOPT_VERBOSE => true );
...
throw new MyException( 
sprintf( _esc( 'URL "%s" doesn`t seem valid. Check your settings.' ), $options[CURLOPT_URL] ) );
}
}
...
$curl_options = $http_options + $ftp_options + $ssl_options + $proxy_options + $global_options +
$factory_defaults; 
return $curl_options;
}
...
} while ( false === $result && ( $count-- ) > 0 &&
( 0 === $is_ftp_bitmask || $is_ftp_bitmask > 0 && 8 == $error_no ) ); 
...
sprintf( 
_esc( '[%s] Curl error code=%d, HTTP code=%d' ) . PHP_EOL, 
...
trigger_error( 
_esc( 
'This should never happen. CURL_DEBUG is on but CURL_DEBUG_LOG is not defined. That is strange!' ), 
E_USER_WARNING );
...
isset( $curl_options[CURLOPT_PROXYTYPE] ) && $proxy = ( in_array( 
$curl_options[CURLOPT_PROXYTYPE], 
CURLPROXY_HTTP, 
CURLPROXY_HTTP_1_0 ) ? 'http' : 'tcp' ) . '://';
...
isset( $curl_options[CURLOPT_FOLLOWLOCATION] ) && $curl_options[CURLOPT_FOLLOWLOCATION] &&
$context_options['follow_location'] = true;
isset( $curl_options[CURLOPT_TIMEOUT] ) && $context_options['timeout'] = $curl_options[CURLOPT_TIMEOUT];
isset( $curl_options[CURLOPT_USERAGENT] ) &&
$context_options['user_agent'] = $curl_options[CURLOPT_USERAGENT];
isset( $curl_options[CURLOPT_CUSTOMREQUEST] ) &&
$context_options['method'] = $curl_options[CURLOPT_CUSTOMREQUEST];
}
return $this->post( 
$url, 
$header, 
...
$opened_files = array();
$this->_initConnHandle();
$cmd_options = array( CURLOPT_URL => $url );
if ( ! empty( $curl_options ) )
...
$callback_ptr = array( $callback_info[2], $callback_info[3] );
...
if ( isset( $postfields ) ) {
$cmd_options += array( CURLOPT_POST => 'POST' == $method, CURLOPT_POSTFIELDS => $postfields );
}
if ( null != $progress_callback ) {
$cmd_options += array( CURLOPT_NOPROGRESS => false, CURLOPT_PROGRESSFUNCTION => $progress_callback );
}
...
private function _deprecated_curlPOST( 
&$url, 
$header = null, 
&$postfields = null, 
$outfile = null, 
...
( is_array( $postfields ) ? http_build_query( $postfields ) : $postfields );
$postfields = null;
}
}
...
$this->_ssl_cached_cert_info = array( 'version' => $ssl_version, 'certificate' => $ssl_server );
}
...
$secure_proto = array( CURLPROTO_HTTPS, CURLPROTO_FTPS, CURLPROTO_SCP, CURLPROTO_SFTP );
$is_secure = false;
