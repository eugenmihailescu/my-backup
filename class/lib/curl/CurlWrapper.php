 ...
 * 
 * @id      : CurlWrapper.php | Mon Sep 14 21:14:57 2015 +0200 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
if (0 != $errcode)
throw new MyException ( sprintf ( _esc ( 'An unexpected error occured while posting data to %s:<br>%s' ), $url, $errmsg ), $errcode );
...
CURLOPT_SSL_VERIFYHOST => $this->_ssl_chk_host  // when CURLOPT_SSL_VERIFYPEER=false this option doen't really matter
);
...
$this->_logfile->writeLog ( sprintf ( _esc ( '[%s] Curl error code=%d, HTTP code=%d' ) . PHP_EOL, date ( DATETIME_FORMAT ), $error_no, $http_code ) );
} else
