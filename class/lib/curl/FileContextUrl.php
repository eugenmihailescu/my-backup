 ...
 * 
 * @id      : FileContextUrl.php | Tue Feb 16 15:27:30 2016 +0100 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
$this->_logfile->writeLog ( sprintf ( '[%s] URL=%s' . PHP_EOL, date ( DATETIME_FORMAT ), $url ) );
...
$this->_logfile->writeLog ( sprintf ( 'Options=%s' . PHP_EOL, print_r ( $exec_options, true ) ) );
! empty ( $this->_debug_buffer ) && $this->_logfile->writeLog ( $this->_debug_buffer . PHP_EOL );
isset ( $response_meta ) && $this->_logfile->writeLog ( sprintf ( _esc ( '[%s] Curl error code=%d, HTTP code=%d' ) . PHP_EOL, date ( DATETIME_FORMAT ), isset ( $error ['code'] ) ? $error ['code'] : 0, $response_meta ['status'] ['code'] ) );
$this->_logfile->writeLog ( str_repeat ( '-', 80 ) . PHP_EOL );
} else
