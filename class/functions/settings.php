 ...
 * @file    : settings.php $
 * 
 * @id      : settings.php | Tue Dec 13 06:40:49 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$java_scripts[] = 'parent._addEventListener(window,"unload",function(e){if (true==parent.globals.JOB_RUNNING){parent.asyncRunJob(parent.ajaxurl, "action=abort_job&nonce=' . wp_create_nonce_wrapper('abort_job') . '", null, null, null, 3, parent.dummy, -1, null);}});';
...
function register_settings($callback, $priority = 10)
{
global $registered_settings;
if (_is_callable($callback)) {
isset($registered_settings[$priority]) || $registered_settings[$priority] = array();
$registered_settings[$priority][] = $callback;
}
}
...
$chk_invalid_paths[$upload_path] = array(
'disk',
'disk_path_id'
);
...
$message = _esc('The following options which are pointing to restricted paths (open_basedir is in effect) were automatically changed to:') . '<ol class="alert-list">';
...
$message = sprintf(_esc('The `Upload max chunk size` option has an invalid value. It was set automatically to %s'), getHumanReadableSize($min_chunk_size * 1024));
add_alert_message($message);
}
if ($is_wp) {
...
ksort($registered_settings);
foreach ($registered_settings as $callbacks) {
...
$log_file->writelnLog(sprintf("%s - %s", date(DATETIME_FORMAT), _esc('[!] action won`t trigger ; it`s probably hijacked')));
return;
}
