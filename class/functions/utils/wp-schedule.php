 ...
 * 
 * @id      : wp-schedule.php | Tue Feb 7 08:55:11 2017 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$result .= "<tr $style><td rowspan='" . (1 + count($job)) . "' style='text-align:center'>@timestamp@</td><td colspan='4' $style></td></tr>"; 
...
$img = sprintf("<img id='img_%s' src='%s' style='display:none;cursor:pointer;' onclick='%s' title='%s'>", $uniq_id, plugins_url_wrapper('img/save.png', IMG_PATH), $img_click, $img_title);
...
$timestamp = sprintf("<img id='imgrm_%s' style='cursor:pointer' src='%s' onclick='jsMyBackup.remove_schedule(\"%s\")' title='%s'/>", $uniq_id, plugins_url_wrapper('img/user-trash.png', IMG_PATH), $hook_name, $img_title) . $timestamp;
}
...
"\n",
PHP_EOL
), '', ob_get_clean()) . '<table style="width:100%"><tr><td>' . readMoreHere('http://theme.fm/2011/11/wordpress-internals-the-cron-2715') . '</td><td style="text-align:right">' . sprintf(_esc('Local time: %s'), date(DATETIME_FORMAT . ' e')) . '</td></tr></table>';
}
function get_schedule_list($message = null, $encode = true)
{
return sprintf('<p>' . _esc('%sBelow is the table of the enabled WP-Cron schedules:%s') . '</p>', ! empty($message) ? $message . '<br>' : '', str_replace("'", $encode ? "\'" : "'", get_active_wpcrons('/' . WPCRON_SCHEDULE_HOOK_NAME . '|' . WPMYBACKUP_LOGS . '_\d+/')));
}
...
$_logfile instanceof LogFile && $_logfile->writelnLog(sprintf(_esc("[%s] %s Cron schedule %s"), date(DATETIME_FORMAT), WPMYBACKUP, strip_tags($opstr)));
if (false !== ($timestamp = _call_user_func(($activate ? 'activate' : 'deactivate') . '_schedule', $_logfile, $recurrance, $cron_hook, $timestamp))) {
$message = sprintf(_esc('The WP Cron %s backup job was %s%s.'), '<b>' . $recurrance . '</b>', $opstr, $activate ? _esc(' at ') . getSpanE(date(DATETIME_FORMAT, $timestamp), null, null, 'yellow') : '');
