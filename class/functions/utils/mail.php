 ...
 * @file    : mail.php $
 * 
 * @id      : mail.php | Mon Dec 28 17:57:55 2015 +0100 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
function sendMail($from, $to, $subject, $body, $attachments = null, $plain_text = null, $priority = 3, $driver = null, $params = null, $debug = false) {
$eol = "\r\n";
$deol = $eol . $eol;
...
$stripped_styles = preg_replace ( '/<((style|script)>)[\s\S]*?<\/\1/', '', $body );
$plain_text = empty ( $plain_text ) ? strip_tags ( $stripped_styles, '<a></a>' ) : $plain_text;
$plain_text = preg_replace ( '/<a[\s\S]*?href\s*=\s*[\'"](.*?)[\'"][\s\S]*?>([\s\S]*?)<\/a>/', '$2 ($1)', $plain_text );
...
$scripts_style = '';
preg_match_all ( '/<((style|script)>)([\s\S]*?)<\/\1/', $body, $matches ) && $scripts_style = implode ( PHP_EOL, $matches [0] );
$body = preg_replace ( '/<((html|head|body)>)[\s\S]*<\/\1/', '', $body ); 
$body = preg_replace ( '/<meta[^>]*>/', '', $body ); 
echo '<html>', $eol;
echo '<head><meta http-equiv="content-type" content="', $html_mime, '";', $charset_tag, '>' . $scripts_style . '</head>', $eol;
echo '<body>', $eol;
...
if (is_array ( $file_item ) && is_file ( $file_item ['tmp_name'] )) {
...
$result = true;
$debug && file_put_contents ( SMTP_DEBUG_LOG, $log_sep . sprintf ( '[%s] %s%s', date ( DATETIME_FORMAT ), $to, is_array ( $attachments ) ? ' (' . count ( $attachments ) . ' attachments)' : '' ) . $log_sep, FILE_APPEND );
if (! $native_backend) {
...
if ($debug) {
! empty ( $params ) && file_put_contents ( SMTP_DEBUG_LOG, sprintf ( 'Connection params : %s', print_r ( $params, true ) ) . PHP_EOL, FILE_APPEND );
file_put_contents ( SMTP_DEBUG_LOG, sprintf ( 'Message size : %d bytes', strlen ( $html_message ) ) . PHP_EOL, FILE_APPEND );
file_put_contents ( SMTP_DEBUG_LOG, sprintf ( 'Attachment size : %d bytes', $attachment_size ) . PHP_EOL, FILE_APPEND );
file_put_contents ( SMTP_DEBUG_LOG, sprintf ( PHP_EOL . '[%s] Status : %s', date ( DATETIME_FORMAT ), true !== $result ? $result : 'success' ) . $log_sep, FILE_APPEND );
}
if (true !== $result) {
