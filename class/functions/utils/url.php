 ...
 * @file    : url.php $
 * 
 * @id      : url.php | Tue Dec 13 06:40:49 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
{
return str_replace(str_replace('/', DIRECTORY_SEPARATOR, realpath($_SERVER['SCRIPT_NAME'])), '', realpath($_SERVER['SCRIPT_FILENAME']));
}
function stripUrlParams($url, $varname)
{
if (empty($varname))
return $url;
is_string($varname) && $varname = array(
$varname
);
...
if (empty($url))
return $url;
is_string($param_name) && $param_name = array(
$param_name
);
is_string($new_value) && $new_value = array(
$new_value
);
...
return sprintf('%s://%s%s/%s', $protocol, $server, $port, ! empty($uri) && '/' == $uri[0] ? substr($uri, 1, strlen($uri) - 1) : $uri);
}
...
<tr>
<td><img
src="<?php echo plugins_url_wrapper('img/js.png', IMG_PATH);?>"></td>
...
substr($self_url, - 1) == substr($file_relpath, 0, 1) && $self_url = substr($self_url, 0, strlen($self_url) - 1);
...
function checkDownloadWhiteSpace($service = 'test')
{
...
'action' => $action,
'service' => $service,
'nonce' => wp_create_nonce_wrapper($action, true)
);
$url = getAsyncRunURL(); 
try {
