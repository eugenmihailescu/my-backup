...
define ( 'DEFAULT_JSPOPUP_WIDTH', 545 ); 
...
'<b>Array</b>',
getSpan ( '=>', 'gray', null, null, $escaped ) 
), $s );
...
return sprintf ( "<table %s %s><tr style='font-weight:bold;text-align:center;background-color:#f0f0f0;'><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", ! empty ( $id ) ? "id='$id'" : '', $hidden ? "style='display:none'" : '', _esc ( 'Short' ), _esc ( 'Long' ), _esc ( 'Argument value' ), _esc ( 'Description' ) );
};
...
! empty ( $param_options [1] ) && isset ( $param_options [3] ) && $options [str_replace ( ':', '', $param_options [1] )] = $param_options [3];
}
...
$out .= (! $html_format ? $eol : '<table><tr><td colspan="3"><b>') . _esc ( "Options:" ) . (! $html_format ? $eol : '</b></td></tr>');
$header = $html_format ? $table_header () : '';
$footer = $html_format ? '</table>' : '';
$out .= $header;
if (null !== $long_opts)
foreach ( $options as $longname => $description ) {
if (false !== ($key = $arg_opts ( $longname )) && isset ( $short_opts [$key] ) && ! empty ( $short_opts [$key] ))
$short = '-' . str_replace ( ':', '', $short_opts [$key] ) . ',';
else
$short = '';
$arg_nfo = $arg_info ( $long_opts [$key] );
! $html_format && $out .= $TAB . sprintf ( "%-5s --%-20s : %s %s" . $eol, $short, $longname, $description, empty ( $arg_nfo ) ? '' : "($arg_nfo)" );
$html_format && $out .= sprintf ( "<tr><td style='color:blue'>%s</td><td>--%s</td><td>%s</td><td>%s</td></tr>", $short, $longname, $arg_nfo, $description );
}
$out .= $footer;
...
if (count ( $diff ) > 0) {
$tbl_id = uniqid ( 'tbl_' );
$out .= $eol . ($html_format ? "<div style='padding:3px;background-color:#f0f0f0;color:#00ADEE;cursor:pointer' onclick='var d=document.getElementById(&quot;$tbl_id&quot;);d.style.display=&quot;none&quot;==d.style.display?&quot;block&quot;:&quot;none&quot;;'>" : '') . sprintf ( _esc ( 'Furthermore, there are (at least) %d other undocumented parameters' ), count ( $diff ) ) . ($html_format ? '</div>' : '') . $eol;
$out .= $table_header ( $tbl_id, true );
foreach ( $diff as $longname => $key ) {
$short = str_replace ( ':', '', false !== ($k = array_search ( $key, $long_opts )) && isset ( $short_opts [$k] ) ? $short_opts [$k] : '' );
! $html_format && $out .= $TAB . sprintf ( "%-5s --%-15s" . $eol, $short, $longname );
$html_format && $out .= sprintf ( "<tr><td style='color:blue'>%s</td><td>--%s</td><td>%s</td></tr>", $short, $longname, $arg_info ( $key ) );
}
$out .= $footer;
}
$out .= $eol . sprintf ( _esc ( "Send bug reports at %s" ), ($html_format ? "<a href='mailto:" . getPluginAuthorEmail () . "'>" : '') . getPluginAuthorEmail () . ($html_format ? '</a>' : '') ) . $eol;
...
return ($quote_enclosed ? '"' : '') . "js55f82caaae905.popupWindow('" . (empty ( $title ) ? _esc ( 'Help' ) : $title) . "'," . $msg . ',' . DEFAULT_JSPOPUP_WIDTH . ($reuse_div ? 'null,null,' . $reuse_div : '') . ");" . ($quote_enclosed ? '"' : '');
}
function getPopup($caption, $msg, $escaped = true) {
$escape_char = $escaped ? "\\" : '';
return "<a class=$escape_char'help$escape_char' onclick=$escape_char'js55f82caaae905.popupWindow(&quot;" . _esc ( 'Help' ) . "&quot;,$msg," . DEFAULT_JSPOPUP_WIDTH . ");$escape_char'>$caption</a>";
}
...
return sprintf ( '<red>[!] : %s</red>' . (defined ( 'PHP_DEBUG_ON' && PHP_DEBUG_ON ) ? ' (%s:%d)' : ''), (empty ( $sender ) ? '' : $sender . '://') . $e->getMessage (), basename ( $e->getFile () ), $e->getLine () );
}
...
return $popup ? getPopup ( $name, '&quot;' . $anchor . '&quot;' ) : escape_quotes ( '<a href="' . $anchor . '" target="' . $target . '">' . $name . '</a>', $escape );
}
}
...
return sprintf ( _esc ( 'Read more %s' ), getAnchor ( empty ( $about_str ) ? _esc ( 'here' ) : $about_str, $url, $target, $escape ) );
}
...
! empty ( $font_style ) && $style [] = "font-style:$font_style";
...
$script = "<script>var el=document.getElementById('$id');js55f82caaae905.addHeaderToggle(el.getElementsByTagName('h4'),true,'" . (empty ( $img_path ) ? plugins_url_wrapper ( 'img/', IMG_PATH ) : $img_path) . "');" . ($collapsed ? '' : "el.getElementsByTagName('h4')[0].click();") . '</script>';
