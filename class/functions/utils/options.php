 ...
 * @file    : options.php $
 * 
 * @id      : options.php | Wed Sep 16 11:33:37 2015 +0200 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
$db [$option] = $new_value;
mkdir ( _dirname ( LOCAL_OPTION_DB_PATH ) );
...
global $java_scripts;
if (defined ( 'PHP_DEBUG_ON' )) 
return;
define ( 'PHP_DEBUG_ON', strToBool ( $settings ['debug_on'] ) ); 
define ( 'CURL_DEBUG', strToBool ( $settings ['curl_debug_on'] ) ); 
define ( 'YAYUI_COMPRESS', strToBool ( $settings ['yayui_on'] ) ); 
define ( 'STATISTICS_DEBUG', strToBool ( $settings ['stats_debug_on'] ) ); 
define ( 'DEBUG_STATUSBAR', strToBool ( $settings ['debug_statusbar_on'] ) ); 
define ( 'SMTP_DEBUG', strToBool ( $settings ['smtp_debug_on'] ) ); 
if (! $ajax) {
...
$js_script = array ();
if (is_wp ()) {
$settings_trigger = array (
'schedule_wp_cron',
'schedule_enabled',
'schedule_grp',
'schedule_wp_cron_time' 
);
...
$result = change_schedule ( $_logfile, $cron_schedule, $is_activated, isset ( $new_settings ['schedule_wp_cron_time'] ) && ! empty ( $new_settings ['schedule_wp_cron_time'] ) ? strtotime ( $new_settings ['schedule_wp_cron_time'] ) : null );
} catch ( MyException $e ) {
...
$result [] = update_wp_config ( array (
'WP_DEBUG' => boolToStr ( $wp_debug_on ) 
) );
if ($wp_debug_on) {
