 ...
 * @file    : history.php $
 * 
 * @id      : history.php | Tue Feb 7 08:55:11 2017 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
global $java_scripts;
$java_scripts[] = sprintf("jsMyBackup.popupError('%s','%s');", _esc('DB Upgrade Error'), implode('.<br>', array_unique($success)));
}
};
if (null == $opts)
$opts = $settings;
...
$query .= (empty($query) ? '' : ',') . TBL_PREFIX . $table . '.' . implode(',' . TBL_PREFIX . $table . '.', $fields);
}
$query = 'SELECT ' . $query;
$from = ' FROM ' . TBL_PREFIX . TBL_JOBS;
$from .= ' LEFT OUTER JOIN ' . TBL_PREFIX . TBL_FILES . ' ON ' . TBL_PREFIX . TBL_JOBS . '.id=' . TBL_PREFIX . TBL_FILES . '.jobs_id';
$from .= ' LEFT OUTER JOIN ' . TBL_PREFIX . TBL_SOURCES . ' ON ' . TBL_PREFIX . TBL_SOURCES . '.jobs_id=' . TBL_PREFIX . TBL_FILES . '.jobs_id AND ' . TBL_PREFIX . TBL_SOURCES . '.id=' . TBL_PREFIX . TBL_FILES . '.sources_id';
$from .= ' LEFT OUTER JOIN ' . TBL_PREFIX . TBL_STATS . ' ON ' . TBL_PREFIX . TBL_JOBS . '.id=' . TBL_PREFIX . TBL_STATS . '.jobs_id AND ' . TBL_PREFIX . TBL_STATS . '.files_id=' . TBL_PREFIX . TBL_FILES . '.id AND ' . TBL_PREFIX . TBL_STATS . '.action=1 AND ' . TBL_PREFIX . TBL_STATS . '.error IS NULL';
...
$result['files'][$data['source_type']][$data['id']] = array(
$data['filename'],
$data['filesize'],
$operation
);
...
$result['jobsize'] = array_reduce($result['files'], function ($carry, $item) {
$result = $carry;
foreach ($item as $array)
$result += $array[1];
return $result;
}, 0);
array_walk($result['files'], function (&$item, $key) {
foreach ($item as $file_id => $file_info)
$item[$file_id][1] = getHumanReadableSize($file_info[1]);
});
$result['source_type'] = array();
$rst = $stat_mngr->queryData('select source_type FROM ' . TBL_PREFIX . TBL_SOURCES . ' WHERE jobs_id=' . $job_id . ' AND source_type IS NOT NULL GROUP BY source_type');
while ($data = $stat_mngr->fetchArray($rst))
...
return (empty($carry) ? '' : ($carry . ' AND ')) . (preg_match('/^\w+[\w\d_]=/', $item) ? TBL_PREFIX . TBL_JOBS . '.' : '') . $item;
}, '');
empty($where) || $where = ' WHERE ' . $where;
$rst = $stat_mngr->queryData('SELECT id FROM ' . TBL_PREFIX . TBL_JOBS . $where . ' ORDER BY started_time DESC LIMIT 1;');
$data = $stat_mngr->fetchArray($rst, 1);
$job_id = $data['id'];
return array(
$job_type => getJobInfo($stat_mngr, $job_id)
);
}
...
$rst = $stat_mngr->queryData('SELECT SUM(B.completed) AS completed, SUM(B.partial) AS partial, SUM(B.failed) AS failed, SUM(B.files_count) AS files_count, SUM(B.job_size) AS file_size, SUM(B.data_size) AS data_size, AVG(B.ratio) as ratio FROM (SELECT CASE WHEN A.job_status = 2 AND A.job_state = 0 THEN 1 ELSE 0 END AS completed, CASE WHEN A.job_status in (1,2) AND A.job_state = 1 THEN 1 ELSE 0 END AS partial, CASE WHEN A.job_status=3 OR A.job_state in (null,2) THEN 1 ELSE 0 END AS failed, A.files_count, A.job_size, A.ratio, (SELECT SUM(B.filesize) FROM ' . $tbl_files . ' B WHERE B.jobs_id = A.id) AS data_size FROM ' . $tbl_jobs . ' A WHERE A.job_type = ' . $job_type . ') B');
