 ...
 * 
 * @id      : WPBackupHandler.php | Mon Dec 28 17:57:55 2015 +0100 | Eugen Mihailescu <eugenmihailescux@gmail.com> $
*/
...
sprintf( 
_esc( 
"<red>[!] file %s cannot be uploaded to %s due to insufficient disk space (free: %s, needs: %s)</red>" ), 
$filename, 
$target_name, 
getHumanReadableSize( $free_space ), 
...
sprintf( 
_esc( "<yellow>file %s (%s) exceeds %s limit of %s</yellow>" ), 
basename( $filename ), 
getHumanReadableSize( $filesize ), 
$target_name, 
getHumanReadableSize( $upload_limit ) ), 
BULLET );
$parts = splitFile( $filename, $upload_limit );
$this->outputError( 
sprintf( _esc( "file %s was splitted into %d parts" ), basename( $filename ), count( $parts ) ), 
false, 
$err_params );
} else
...
$part, 
$add_filename ? addTrailingSlash( $path, '/' ) . basename( $part ) : $path );
if ( ! $api->curlAborted() ) {
...
sprintf( 
_esc( 
"<red>[!] file %s cannot be moved to disk due to insufficient disk space (free: %s, required: %s)</red>" ), 
$arc, 
getHumanReadableSize( $free ), 
...
$arcs = $mysqlbkp->compressMySQLScript( 
$this->getBackupName() . '.' . $params['mysql_format'], 
...
array( 
array( $this, 'onNewArc' ), 
array( $this, 'startCompress' ), 
array( $this, 'onMySqlMaint' )
,null, 
...
getHumanReadableSize( $memory_limit ), 
...
$this->outputError( 
sprintf( 
'<red>' . _esc( 'Cannot send backup via e-mail due to invalid e-mail address' ) . '</red> [%s]', 
empty( $to ) ? _esc( 'empty' ) : $to ), 
false, 
$err_params );
}
}
...
$dates = getDatesByAge( 
getFilesTime( $files, $this->getBackupName() . '-' ), 
$this->_getTargetMaxAge( array( 'DROPBOX_TARGET', 'GOOGLE_TARGET', 'WEBDAV_TARGET' ) ), 
OLDEST );
$orphans = array();
foreach ( $dates as $d )
...
$deleted_files = 0;
$this->logOutputTimestamp( 
sprintf( _esc( "Cleaning-up %s files older than %d days" ), ucwords( $TARGET_NAMES[$target] ), $days ) );
try {
...
$delete_func = array( $api, 'deleteFile' );
}
if ( ! is_array( $metadata ) )
throw new MyException( 
sprintf( _esc( 'Clean-up old archives not implemented for %s' ), $TARGET_NAMES[$target] ) );
$files = array();
...
if ( ! $this->getEnabledTargets( array( 'DROPBOX_TARGET', 'GOOGLE_TARGET', 'WEBDAV_TARGET' ) ) )
return false;
$ext = $this->getCompressionName();
$files = getFileListByExt( $this->getWorkDir(), $ext );
$dates = getDatesByAge( 
...
if ( $tar_limit > 0 )
$multi_vol = sprintf( 
_esc( ", multi-volume @ max %s/volume" ), 
getHumanReadableSize( $tar_limit ) );
else
$multi_vol = '';
...
$this->logOutputTimestamp( 
sprintf( 
'<yellow>[%d/%d] ' . _esc( 'Skipping source folder' ) . ' %s</yellow>', 
++$section_done, 
...
$arcs = $this->_runExternalCompressTool( 
$archive_name, 
$tar_limit, 
$dir_path, 
$section_temp_file, 
$file_section_info['lines'], 
$excl_dirs );
} else
$arcs = $this->_runInternCompressTool( 
$archive_name, 
$tar_limit, 
$section_temp_file, 
$file_section_info['lines'] );
...
_esc( '%s files added (%s) to %d archive(s) out of %d scheduled files from %s' ), 
getSpan( $total_fcount, '#fff', 'bold' ), 
...
$msg = _esc( 
'(the added files count may be different than the estimated/scheduled file count; this is a known behaviour)' );
...
private function _runExternalCompressTool( 
$archive_path, 
$archive_limit, 
$src_dir, 
$temp_file, 
...
$this->logOutputTimestamp( 
sprintf( 
_esc( "archiving %s to %s" ), 
getHumanReadableSize( $archive_size ), 
preg_replace( "/.tar$/", '.*.tar', basename( $archive_path ) ) ), 
"[*]" );
else
$this->logOutputTimestamp( 
sprintf( 
...
strToBool( $this->options['relative_path'] ) && $alias = str_replace( 
delTrailingSlash( @constant( 'ABSPATH' ) ? ABSPATH : ROOT_PATH, '/' ), 
'', 
$alias );
$this->getVerbosity( VERBOSE_FULL ) &&
$this->logOutputTimestamp( sprintf( _esc( "adding %s" ), $file ), BULLET );
...
$this->outputError( 
sprintf( 
'<red>' . _esc( '%s does not exists. This should never happen.' ) . '</red>', 
basename( $arc ) ), 
false, 
$err_params );
return;
...
if ( ( - 100 != ( $target = $this->getTargetConstant( $target ) ) ) &&
null != ( $found_target = $this->getTarget( $target ) ) && $found_target->isEnabled() ) {
list( $target_name, $oper_send, $oper_sent ) = $this->getTargetOperConsts( $target );
...
if ( null != $errmsg )
$this->outputError( 
sprintf( '<red>%s</red>', $errmsg['message'] ), 
false, 
$err_params );
}
}
...
$this->outputError( 
sprintf( _esc( "<red>Internal error: onNewArc() called without arguments.</red>" ), basename( $arc ) ), 
false, 
$err_params );
return $result;
}
...
( null !== ( $target = $this->getTarget( $i ) ) && $target->isEnabled() );
if ( $do_cleanup ) {
$this->_uploadOrphans();
$this->_cleanupOrphans();
global $BACKUP_TARGETS;
foreach ( array_keys( $BACKUP_TARGETS ) as $target )
null != ( $target_obj = $this->getTarget( $target ) ) && $target_obj->isEnabled() &&
$cleaned += $this->_cleanUpOldArchives( $target );
}
$status = $ok_status;
$this->setError( null );
}
...
$this->logSeparator();
$job_summary_str = $prefix . sprintf( 
_esc( '<b>Job finished %s</b>.<br>Total elapsed time %s' ), 
$status, 
timeFormat( $elapsed_time ) );
$ok_status == $status && $job_summary_str .= sprintf( 
_esc( '<br>Average compression ratio %.2fx' ), 
0 != $this->compressed ? $this->uncompressed / $this->compressed : 0 );
...
$this->logOutput( 
sprintf( 
'<div class="hintbox rounded-container"><black>' . _esc( 
'Do not forget to test your backups regularly. This step is as important as the backup itself.' ) . ' ' .
readMoreHere( 'http://www.taobackup.com/testing_info.html' ) . '.</black></div>' ) );
$this->logSeparator();
...
$result = $mysqlbkp->execTableMaintenance( 
$tables, 
array( $this, 'onMySqlMaint' ), 
array( $this, 'onProgress' ), 
array( $this, 'chkProcessSignal' ) );
$this->logOutputTimestamp( 
sprintf( _esc( 'In total %d tables have been checked.' ), count( $result ) ), 
BULLET );
...
$this->getProgressManager()->cleanUp();
$this->logOutputTimestamp( 
sprintf( _esc( '%s<b>Task completed %s</b>.%s' ), $prefix, $status, $sufix ), 
BULLET, 
0 );
}
