 ...
 * 
 * @id      : AbstractJob.php | Wed Dec 7 18:54:23 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$this->exclude_ext = ($o = getParam($opts, "excludeext", implode(',', $COMPRESSION_NAMES))) ? explode(",", $o) : array();
...
$this->exclude_files[$key] = @constant(__NAMESPACE__ . '\\' . substr($value, 1, strlen($value) - 2));
...
if ($operation < 0)
return sprintf('%s %s %s ' . _esc('on') . ' %s%s', $opstr, $secure_str, getSpan(basename($filename), 'cyan'), getSpan($protocol . '://', 'mangenta'), shorten_path($path));
return sprintf('%s %s %s (%s) %s %s%s', $opstr, $secure_str, getSpan(basename($filename), 'cyan'), getHumanReadableSize($filesize < 0 ? filesize($filename) : $filesize), $download ? 'from' : 'to', getSpan($protocol . '://', 'magenta'), shorten_path($path));
}
...
$msg = sprintf(_esc("Access denied due to concurrent job (%s started %d seconds ago)"), $last_job_str, time() - $last_job_timestamp);
...
if ($rst = $this->_statistics_manager->queryData('SELECT id,started_time FROM ' . TBL_PREFIX . TBL_JOBS . ' WHERE job_type=' . $job_type . (false !== $mode ? ' AND mode=' . $mode : '') . ' ORDER BY started_time DESC LIMIT 1;')) {
...
$this->logOutputTimestamp(sprintf("<yellow><b>" . _esc('TAR volume(#%d) %s successfully (%s @ ratio %.2f => %s)') . "</b></yellow>", $vol_no, $decompress ? _esc('decompressed') : _esc('compressed'), $decompress ? $fs : $us, $ratio > 0 ? sprintf('%.2f', $ratio) : _esc('unknown'), $decompress ? $us : $fs), BULLET, 2);
if (null !== $this->_statistics_manager) {
$stat_data = array(
...
if (! empty($name) && (null === $opt && null !== getParam($this->options, $name)) || (null !== $opt && $filter == $opt))
return $filter;
}
return NONE;
}
...
{
isset($this->_added_targets[$target]) && ($this->_added_targets[$target] += $count) || $this->_added_targets[$target] = $count;
}
...
protected function stopTransfer($operation, $filename, $filesize, $uncompressed = 0, $action = 'METRIC_ACTION_TRANSFER')
{
$end = time();
...
isset($stat_data['JOBTBL_FILE_CHECKSUM']) || $stat_data['JOBTBL_FILE_CHECKSUM'] = file_checksum($filename);
is_array($this->process_data) && $stat_data = array_merge($stat_data, $this->process_data);
$this->_statistics_manager->addJobData($this->_jobs_id, $stat_data);
}
return $duration;
...
$oper_send = @constant(__NAMESPACE__ . '\\OPER_' . ($download ? 'GET' : 'SEND') . '_' . strtoupper($target_name));
$oper_sent = @constant(__NAMESPACE__ . '\\OPER_' . ($download ? 'GOT' : 'SENT') . '_' . strtoupper($target_name));
return array(
$target_name,
$oper_send,
$oper_sent
...
(! $download || $filesize >= 0) && $result['METRIC_SIZE'] = $filesize >= 0 ? $filesize : (_file_exists($filename) ? filesize($filename) : 0);
return $result;
}
...
if (! $this->getTarget($target)->isEnabled() || empty($filename) || ! ($download || _file_exists($filename)))
return true;
...
return false;
$this->logOutputTimestamp(sprintf(_esc('%s the file %s with %s'), $encrypt ? _esc('encrypting') : _esc('decrypting'), $filename, str_replace(__NAMESPACE__ . '\\', '', get_class($enc))), BULLET);
...
$this->logOutputTimestamp(sprintf('<b><yellow>' . _esc("%s job started with %s interface (%s)") . '</yellow></b>', $this->getJobTypeStr($job_type), ! empty($this->sender) ? $this->sender : (is_cli() ? "CLI" : "WP-Admin"), APP_VERSION_ID));
...
$this->logOutputTimestamp(_esc("Log directory") . " : " . shorten_path(getBranchedFileName(LOG_DIR)), '*', 1);
...
$this->logOutputTimestamp(_esc("MySQL extension") . " : " . (empty($mysq_ext) ? _esc('best available') : $mysq_ext), '*', 1);
...
$msg = sprintf('<red>[!] : %s</red>' . (defined(__NAMESPACE__.'\\PHP_DEBUG_ON' && PHP_DEBUG_ON) ? ' (%s:%d)' : ''), $error_get_last['message'], basename($error_get_last['file']), $error_get_last['line']);
else
return;
} else
$msg = $str;
...
$this->addMessage(false !== strpos($str, '<yellow>') ? MESSAGE_TYPE_WARNING : MESSAGE_TYPE_ERROR, html2Text($msg), empty($this->_jobs_id) ? 0 : $this->_jobs_id);
}
...
return preg_replace('/(\[\d{4}\-\d{2}\-\d{2} \d{2}:\d{2}:\d{2}\])/', '<span style="color:#777">$1</span>', $str);
};
$format_hr = function ($str) {
return preg_replace('/^(\-+)$/m', '<hr>', $str);
};
$format_warnings = function ($str) {
return preg_replace('/(' . _esc('WARNING') . '\s*:)(.*)/mi', '<span style="background-color:#FFB600;color:#FFF;font-weight:bold">$1</span>$2', $str);
};
$format_errors = function ($str) {
return preg_replace('/(\[!\].+)$/m', '<span style="background-color:#FFC0B5">$1</span>', $str);
};
$format_totals = function ($str) {
return preg_replace('/((' . _esc('SUBTOTAL') . '|' . _esc('GRAND TOTAL') . ')\s*:)(.*)/im', '<span style="background-color:#FCFCAF;color:#000"><strong>$1</strong>$3</span>', $str);
};
$format_steps = function ($str) {
return preg_replace('/(\[\d+\/\d+\])/', '<span style="background-color:#00ADEE;color:#FFF">$1</span>', $str);
};
$format_target = function ($str) {
global $BACKUP_TARGETS;
return preg_replace('/((' . implode('|', $BACKUP_TARGETS) . '):\/\/)/', '<span style="background-color:#FFB2FF">$1</span>', $str);
};
...
$items = $mhdl->getMessagesByKeys(array(
'ref_id',
'type'
), array(
$job_id,
...
return '<table style="font-family:fixed;font-size:10px;color:#000;width:100%;border:1px solid tomato;border-radius:5px;margin-top:5px;margin-bottom:5px"><tr style="background-color:tomato;color:#FFF"><td style="padding:5px">' . sprintf(_esc('The job generated the following %d alert messages'), $count) . '</td></tr><tr><td>' . implode('</td></tr><tr><td>', $result) . '</td></tr></table>';
return '';
};
...
$subject = sprintf('%s - %s %s', WPMYBACKUP, strtolower($this->getJobTypeStr()), preg_replace('/.*(\/)\1([^\1]+)/', '$2', get_home_url_wrapper()));
$plain_text = empty($msg) ? $this->getLogBuffer() : $msg;
$title = sprintf(_esc('Below is the %s log for the %s job #%s (%s)'), WPMYBACKUP, strtolower($this->getJobTypeStr()), $this->_jobs_id, selfURL(true));
$body = '<div>';
$body .= '<p style="font-size:16px;font-family: Verdana,Georgia,serif !important;">' . $title . '</p>';
$body .= '<p><span style="background-color:' . $status_color . ';color:white;font-weight:bold;padding:3px;">' . _esc('Status') . ': ' . $status . '</span></p>';
...
$body .= $job_alerts($this->_jobs_id) . '<table style="font-family:fixed;font-size:10px;color:#000;border:1px solid #00adee;border-radius:5px"><tr><td>' . $body_text . '</td></tr></table>';
$body .= '</div>';
if (! sendHtmlFormattedMail($from, $to, $subject, $body, $plain_text, $priority, $this->getOptions()))
