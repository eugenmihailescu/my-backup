 ...
 * @file    : TarArchive.php $
 * 
 * @id      : TarArchive.php | Tue Feb 7 08:55:11 2017 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
if ((empty($filter) && 'tar' != $ext) || (in_array($method, array(
GZ,
BZ2
)) && ! _function_exists($filter . 'open')))
throw new MyException(sprintf(_esc('%s support is not enabled. Check your PHP configuration (php.ini) or contact your hosting provider.'), strtoupper($filter)));
...
throw new MyException(sprintf(_esc('Archive %s has a bad signature. Unsupported format.'), shorten_path($this->getFileName())));
if ((0 == $uncompressed_size) && (GZ == $method))
$uncompressed_size = $get_gz_uncompressed($this->getFileName());
if (! empty($new_name))
$result = $new_name;
else
$result = preg_replace('/(\.(' . implode('|', array_diff($COMPRESSION_NAMES, array(
NONE => 'tar'
))) . '))$/i', '', $this->getFileName());
if (empty($filter) || 'tar' == $ext)
return $result;
...
$bw < $uncompressed_size && $this->_stdOutput(sprintf(_esc('[!] Expected %d bytes to decompress but I got only %d'), $uncompressed_size, $bw));
}
return $result;
}
