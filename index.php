 ...
 * @file    : index.php $
 * 
 * @id      : index.php | Tue Dec 13 06:40:49 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$login_obj = new SimpleLogin ( defined ( __NAMESPACE__.'\\LOG_PREFIX' ) ? dirname ( LOG_PREFIX ) : null, defined ( __NAMESPACE__.'\\SSL_ENFORCE' ) && SSL_ENFORCE );
...
if (! $is_logged) {
$login_js [] = sprintf ( 'jsMyBackup.popupError("%s","%s");', _esc ( 'Login failed' ), _esc ( 'The username or password does not match.' ) );
}
}
...
if ($is_logged) {
define ( __NAMESPACE__."\\WPMYBACKUP_LOGOFF", "<img src=\"" . plugins_url_wrapper ( 'img/avatar.png', IMG_PATH ) . "\"> <a style='cursor:pointer' onclick='jsMyBackup.post(jsMyBackup.this_url,{action:\"logout\"});'>" . sprintf ( _esc ( 'Logoff %s' ), $username ) . "</a>" );
