 ...
 * @file    : functions.php $
 * 
 * @id      : functions.php | Tue Dec 13 06:40:49 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
*/
...
$body .= sprintf(_esc('You have %s days (ie. until %s) to fix this problem.<br>After that the program is automatically deactivated without warning.'), getSpanE(INVALID_LICENSE_LIFESPAN, 'red', 'bold'), date(DATETIME_FORMAT, time() + $remainig_time)) . '<br>';
...
$js .= sprintf(PHP_EOL . 'parent.ajaxurl=%s;' . PHP_EOL, ! is_wp() ? '"' . getAsyncRunURL() . '"' : 'window.ajaxurl');
...
$js .= 'parent._addEventListener(window,(parent.isNull(parent.ie,10)<9?"on":"")+"load",function(){' . implode('', $java_scripts_load) . '});' . PHP_EOL;
if (! empty($java_scripts_beforeunload))
$js .= 'parent._addEventListener(window,parent.isNull(parent.ie,10)<9?"on":"")+"beforeunload",function(){' . implode('', $java_scripts_beforeunload) . '});' . PHP_EOL;
if (! empty($java_scripts_unload))
$js .= 'parent._addEventListener(window,parent.isNull(parent.ie,10)<9?"on":"")+"unload",function(){' . implode('', $java_scripts_unload) . '});' . PHP_EOL;
$js = '<script>' . sprintf(JS_INJECT_COMMENT, 'page') . 'window.jsMyBackup.local=(function(window,parent,undefined){' . $js . '})(this,window.jsMyBackup);' . PHP_EOL . '</script>' . PHP_EOL;
...
if (! defined(__NAMESPACE__."\\YAYUI_HANDLER") && YAYUI_COMPRESS && ("YAYUI_HANDLER") && (empty($_GET) || empty($_GET['noyayui']))) {
...
style="margin: 2px;" class="button-primary"
value="<?php echo $buttons[true];?>"
onclick="<?php echo $accept [true];?>"> <input type="button"
class="button" value="<?php echo $buttons[false];?>"
style="margin: 2px;" onclick="<?php echo $accept [false];?>"></td>
</tr>
</table>
</div>
<?php
...
function insertResponsiveMenu()
{
echo '<div class="app-bar-menus content-container rounded-container">';
insertTabMenusList('',true);
echo '</div>';
}
function insertTabMenusList($id = '', $has_icon = false, $banner = '')
{
global $tab_orientation, $settings;
echo '<ul ', empty($id) ? '' : 'id="' . $id . '"', ' style="list-style-type: none;padding: 0;" onclick="jsMyBackup.block_ui();">';
...
continue;
echo "<li" . (empty($class) ? '' : " class='$class'") . ('notification' == $tab ? ' style="' . ('vertical' == $tab_orientation ? 'border-left-color' : 'border-top-color') . ': red;"' : '') . "><a href='" . $href . "'>";
if ($has_icon)
echo "<div class='menu-item-icon'><img src='" . plugins_url_wrapper('img/menu/'.$tab.'.png', IMG_PATH) . "'></div>";
echo "<div class='menu-item-desc'>$name</div></a>";
...
$visible_tabs = insertTabMenusList('navlist', true, $banner);
echo '</div>';
return $visible_tabs;
}
...
{
global $tab_orientation, $settings;
$export = "jsMyBackup.post(jsMyBackup.ajaxurl,{action:\\'export_settings\\',format:\\'%s\\',nonce:\\'" . wp_create_nonce_wrapper('export_settings') . "\\'});";
...
<td><input type="button" name='dwl_wpmybackup_option'
<?php echo $disabled;?> class="button-primary"
value="<?php _pesc('Export settings');?>"
